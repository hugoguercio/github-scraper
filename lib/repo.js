...
function repo ($, url, callback) {
  ...
  data.watchers = parseInt(badges['0'].children[0].data.replace(/,/g, '').trim(), 10);
  data.stars    = parseInt(badges['1'].children[0].data.replace(/,/g, '').trim(), 10);
  data.forks    = parseInt(badges['2'].children[0].data.replace(/,/g, '').trim(), 10);
  data.commits  = parseInt($('.commits .num').text().replace(/,/g, '').trim(), 10);
  data.branches = parseInt($('.octicon-git-branch').next().text().replace(/,/g, '').trim(), 10);
  data.releases = parseInt($('.octicon-tag').next().text().replace(/,/g, '').trim(), 10);
